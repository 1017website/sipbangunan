...
@endif
    ...
    <div class="hero-stats-bar">
        ...
        <div class="hstat"><div class="hstat-n">{{ $settings['stat_years'] ?? '10+' }}</div><div class="hstat-l">Tahun</div></div>
        <div class="hstat"><div class="hstat-n">{{ $settings['stat_customers'] ?? '1K+' }}</div><div class="hstat-l">Pelanggan</div></div>
    </div>
</div>
    ...
    <section id="contact-sec">
        ...
        <div class="contact-grid">
            <div class="contact-info-card">
                ...
                <div class="cic-body">
                    <div class="ci"><div class="ci-ico">📍</div><div><div class="ci-lbl">Alamat</div><div class="ci-val">{{ $settings['address'] ?? '-' }}</div></div></div>
                    <div class="ci"><div class="ci-ico">📞</div><div><div class="ci-lbl">Telepon / WhatsApp</div><div class="ci-val"><a href="tel:{{ $settings['phone'] ?? '' }}">{{ $settings['phone'] ?? '-' }}</a></div></div></div>
                    <div class="ci"><div class="ci-ico">⏰</div><div><div class="ci-lbl">Jam Operasional</div><div class="ci-val">{{ $settings['hours'] ?? '-' }}</div></div></div>
                    <div class="ci"><div class="ci-ico">✉️</div><div><div class="ci-lbl">Email</div><div class="ci-val"><a href="mailto:{{ $settings['email'] ?? '' }}">{{ $settings['email'] ?? '-' }}</a></div></div></div>
                </div>
                <div class="cic-foot">
                    <a href="https://wa.me/{{ $settings['wa_number'] ?? '6281234567890' }}?text={{ urlencode('Halo ' . ($settings['site_name'] ?? 'SIP Bangunan')) }}" target="_blank" class="wa-cta">
